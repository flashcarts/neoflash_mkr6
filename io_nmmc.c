...
void Neo_OpenSPI( u8 frequency )
{
   CARD_CR1 = 0x0000A000 | frequency;
}

u8 Neo_SPI( u8 dataByte )
{
...
void Neo_CloseSPI ( void )
{
   CARD_CR1 = 0x8000;
}

/*void Neo_MK2GameMode()   {
   Neo_OpenSPI(SPI_FREQ);            // Enable DS Card's SPI port
   Neo_SPI(0xF1);            // Switch to game mode
   Neo_CloseSPI();            // Disable DS Card's SPI port
}

void Neo_MK2PassThroughMode()   {
   Neo_OpenSPI(SPI_FREQ);            // Enable DS Card's SPI port
   Neo_SPI(0xF0);            // Switch to pass through mode
   Neo_CloseSPI();            // Disable DS Card's SPI port
}*/

/*void Neo_EnableEEPROM( u32 enable )   {
...
void Neo_EnableMMC( bool enable, int frequency )
{
   if ( enable == false) {
      Neo_CloseSPI ();
      Neo_SelectMMC (0);
...
Neo_InitMMCRetry:   
   ...
   Neo_EnableMMC( true, 0 );      // Open SPI port to MMC card
   for (i = 0; i < 10; i++) {
...
bool Neo_WriteMMCSector( void *buffer, u32 address )   {
   ...
   Neo_SPI( 0xFF );                                    // Send CRC16 ignore bytes
   //Neo_SPI( 0xFF );                                    // ignored if CRC16 is not used
